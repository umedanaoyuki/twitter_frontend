...
/**
 * サイドバーの本体。現在のパスでメニューを強調するために client component になっている。
 * 未読件数はサーバー側の HomeSidebar から受け取る。
 */
function HomeSidebarNav({ unreadNotificationCount }: HomeSidebarNavProps) {
  // 表示中の画面のメニューを強調する
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <nav aria-label="メインメニュー" className="mt-1 flex flex-col">
          {navItems.map((item) => {
            ...
            // "#" のダミーリンクは pathname と一致しないので自然に非アクティブになる
